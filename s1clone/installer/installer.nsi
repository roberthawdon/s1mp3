...
OutFile "s1clone-setup.exe"

;Default installation folder
InstallDir "$PROGRAMFILES\s1clone"
;Get installation folder from registry if available
InstallDirRegKey HKLM "Software\s1clone" ""
...
!define MUI_FINISHPAGE_RUN "$INSTDIR\s1clone.exe"
...
;!insertmacro MUI_PAGE_LICENSE "..\res\terms.rtf"
...
;VIAddVersionKey /LANG=${LANG_ENGLISH} "LegalTrademarks" "s1clone(T)"
VIAddVersionKey /LANG=${LANG_ENGLISH} "CompanyName" "http://www.s1mp3.de/"
VIAddVersionKey /LANG=${LANG_ENGLISH} "LegalCopyright" "Copyright © 2007-2008 wiRe"
...
SectionGroup /e "s1clone" SEC_LC
  ...
  Section "Core" SEC_LCC
    ...
    WriteRegStr HKLM "Software\s1clone" "" $INSTDIR

    ;Install files
    File "..\Release\s1clone.exe"
    File "files\driver\libusb\libusb0.dll"
   ;File "..\help\s1clone.chm"
    ...
    !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
      ...
      CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\s1clone.lnk" "$INSTDIR\s1clone.exe"
     ;CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\s1clone help.lnk" "$INSTDIR\s1clone.chm"
...
Section -finalize
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\s1clone" "DisplayName" "s1clone"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\s1clone" "UninstallString" "$INSTDIR\uninstall.exe"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\s1clone" "DisplayIcon" "$INSTDIR\s1clone.exe"
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\s1clone" "NoModify" 1
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\s1clone" "NoRepair" 1
...
Section "Uninstall"
  ...
  Delete "$SMPROGRAMS\$STARTMENU_UNINST\s1clone.lnk"
 ;Delete "$SMPROGRAMS\$STARTMENU_UNINST\s1clone help.lnk"
  ...
  DeleteRegKey HKLM "Software\s1clone"
  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\s1clone"
